...
export const find = async (selectColumnObj: Array<keyof Channel>): Promise<Channel[]> => {
...
export const saveOne = async (channel: Partial<Channel>) => {
    ...
    for (const key in channel) {
        const value = queryHelper.fitValueInSql(channel[key as keyof typeof channel]);
 ...
 * @returns 
 */
export const update = async (where: Partial<Channel>, setColmns: Partial<Channel>) => {
